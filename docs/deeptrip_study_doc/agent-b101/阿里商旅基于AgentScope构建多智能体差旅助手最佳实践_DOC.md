...
- 创建时间：2026-04-16 11:00:00
- 更新时间：2026-04-16 11:00:00
- 作者：阿里商旅（整理：徐凯旋）
- 来源：微信公众号
- 原文链接：https://mp.weixin.qq.com/s/3MXwawLU716HM5aGARlTXA
...
- [二、方案](#二方案)
  - [(一) 技术选型](#一-技术选型)
  - [(二) Multi Agent 架构](#二-multi-agent-架构)
  - [(三) 意图识别](#三-意图识别)
  - [(四) 实时思考链 & 流式输出](#四-实时思考链--流式输出)
  - [(五) 上下文工程](#五-上下文工程)
  - [(六) Prompt 工程（工程与智能体结合）](#六-prompt-工程工程与智能体结合)
- [三、周边生态](#三周边生态)
  - [(一) 知识库](#一-知识库)
  - [(二) 观测](#二-观测)
  - [(三) 评测](#三-评测)
...
随着业务逻辑复杂度的持续攀升，Prompt Token 数量急剧增长，token 过多就会导致大模型的注意力机制开始出现明显衰减，无法满足业务对高准确率的核心要求。具体表现为：
...
团队对两个框架进行了多个场景的实操和调研，直观感受：对于基础能力，如 MCP 服务调用、与 Qwen LLM 集成时 AgentScope 开箱即用，LangGraph 要额外做一些适配工作。最终 **选型 AgentScope**，原因如下：
...
通过前置打印钩子函数（print hook），可在 Agent 执行过程中**实时拦截每一条生成的消息**；当捕获到 `tool_use` 类型消息时，自动调用 `add_tool_use`，而检测到 `tool_result` 类型消息时，则触发 `add_tool_result`，从而实现对工具调用及其结果的细粒度追踪与状态同步。
...
**TaskCollector** 是一个**自定义任务状态收集器**，核心职责是**统一管理思考链状态信息**。其主要功能包括：
...
2. 订阅任务状态更新：
   - 监听 `task_update` 类型消息以实时渲染思考链；
   - 监听 `text` 类型消息以展示最终回答。
3. 处理智能体执行结果：
   - 收集最终回答内容；
   - 维护卡片数据与任务的映射关系。
4. 构建结构化的最终响应，并生成符合 SSE 格式的输出流。
...
**选型结论**：经综合评估，Langfuse 在**开源透明**、**数据安全**、**成本控制**和**可扩展性**方面更具优势，完全满足商旅 SaaS + 本地化部署的诉求，是构建可控、可靠的 LLM 应用监控平台的最佳选择。
...
- 阿里商旅 AI 解决方案有幸荣获 **量子位颁发的 2025 人工智能年度杰出解决方案奖**认可
...
> AgentScope 是通义实验室开源的企业级智能体开发框架，提供智能体编排、工具调用与评测等核心能力，并原生支持智能体微调，助力开发者快速搭建企业级 Agent 系统。目前在 GitHub 已经有 16k 的 star，欢迎大家关注使用 AgentScope 探索并沉淀多智能体最佳实践。
>
